  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: async (acceptedFiles) => {
      ...
      if (file) {
        ...
        try {
          const response = await FileUpload(foldername, file); // Call the API to upload the file
          const fileUrl = response.data.url; // Assuming the API returns the URL in the response
          onFileSelect(fileUrl); // Notify parent about the uploaded file URL
          setUploadStatus('success'); // Update status to success
        } catch (error) {
